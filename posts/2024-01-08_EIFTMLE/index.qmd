...
$$
\begin{aligned}
\hat{Q}^1_i &= \hat{Q}(W_i,1) + \hat{\epsilon}\hat{H}_i^1\\
\hat{Q}^0_i &= \hat{Q}(W_i,0) + \hat{\epsilon}\hat{H}_i^0
\end{aligned}
$$
...
The average contrast (treatment vs control) of the updated predictions of mean outcome, across all the units, is the TMLE estimator. 

$$
\hat{\psi}_{\star} = N^{-1}\sum_{i=1}^N (\hat{Q}^1_i-\hat{Q}^0_i)
$$
