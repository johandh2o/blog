...
: Table 2: some doubly/multiply-robust estimation methods {.striped .hover} {tbl-colwidths="\[44,14,14,14,14\]"}
...
**Fix**: *de Aguas, Biele and Pensar's* recoverability criteria via regression adjustment with missing data on $Y$ using pre-treatment set $H$ and forbidden set $M\subset \text{fb}(A,Y;\mathcal{G})$:
...
[**Solution 2: nested/hierarchical regression**:]{style="color:blue;"}
...
This solution has a clear advantage: it does not require modeling the conditional density of $M$, which is a hard job when such a variable is high-dimensional or combines discrete and continuous components. One disadvantage comes from losing one level of multiply-robustness. Plus, a little of math is required in the TMLE front.
...
Combining IPW and regression adjustment solutions produces **multiply-robustness**. A **triply-robust** estimator for the ATE can be constructed for **solution 1**: it will be consistent if all semiparametric models involved are correctly specified, or in one of these scenarios:
...
Using a misspecified model for $M$ (like when reducing its dimension with PCA, VAE or representation learning) leaves the estimator with [worse]{style="color:blue;"} robustness than simply using IPW, as it requires correct specification of $A$, $R_Y$ and $Y$, whereas IPW only requires $A$, $R_Y$

Nested regressions (**solution 2**) would produce **doubly-robustness**, with consistency if either:
