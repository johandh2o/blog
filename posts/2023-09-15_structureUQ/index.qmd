...
Bayesian structure learning is a type of Bayesian inverse problem that aims to infer the posterior distribution of $\mathcal{G}$ given data $\boldsymbol{O}$ [samples from $P(\mathcal{G})$], i.e., $p(\mathcal{G}\mid\boldsymbol{O})$. Many MCMC, VI, and particle-based algorithms have been designed for this purpose, each with its sets of assumptions, strengths and weaknesses.
